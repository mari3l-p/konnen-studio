  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 px-6 py-5">
      <div className="flex items-center justify-between gap-4">
        ...
        {/* Left: title + expand toggle */}
        <div className="flex-1 min-w-0">
          <p className=" text-gray-900 text-base"><span className="font-bold">{pkg.title}</span> {pkg.class_type}</p>
          {hasDetails && (
            <button
        ...
        {/* Right: button */}
        <button className="bg-blue-600 hover:bg-blue-700 text-white font-semibold px-5 py-2.5 rounded-xl text-sm transition-colors shrink-0">
          Seleccionar
        </button>
      </div>
      ...
      {expanded && hasDetails && (
        <div className="mt-4 pt-4 border-t border-gray-100 flex flex-col gap-1.5 text-sm text-gray-600">
          ...
          {pkg.validity && (
            <p className="text-gray-500">Vigencia de {pkg.validity}</p>
          )}
          {pkg.note && (
      ...
      <div className="max-w-3xl mx-auto">
        ...
        {packages.length === 0 ? (
          <div className="bg-blue-50 border border-blue-200 text-blue-700 rounded-2xl px-6 py-5 text-sm">
            Aún no hay paquetes disponibles. Por favor, contacta a tu administrador para obtener más detalles.
          </div>
        ) : (
